#!/usr/bin/env python3
"""Enrichit les articles blog dont le corps fait moins de MIN_WORDS mots.

Insère un ou deux blocs Markdown (passe 1 puis passe 2 si besoin), à partir du
titre, de l’extrait et du slug. Si la ligne exacte `## Vidéo YouTube à intégrer`
existe, les blocs sont insérés **avant** ; sinon ils sont ajoutés **en fin
d’article**. Le frontmatter est détecté par la **première** paire de lignes
...
def word_count_body(body: str) -> int:
    """Compte les « mots » comme des jetons séparés par des espaces contenant
    au moins une lettre ou un chiffre (meilleure approximation du `wc -w` sur
    du français avec apostrophes et ponctuation collée).
    """
    b = re.sub(r"```[\s\S]*?```", " ", body)
    b = re.sub(r"!\[[^\]]*\]\([^)]*\)", " ", b)
    b = re.sub(r"\[([^\]]+)\]\([^)]*\)", r"\1", b)
    ...
    tokens = b.split()
    return sum(
        1 for t in tokens if re.search(r"[0-9A-Za-zÀ-ÖØ-öø-ÿ]", t, re.UNICODE)
    )
...
def parse_frontmatter(
    ...
    """Retourne (champs parsés, corps, bloc frontmatter exact jusqu’au 2e ---)."""
    ...
    for line in fm_text.splitlines():
        m = re.match(r"^(title|excerpt|category):\s*(.+)$", line)
        if m:
            k, v = m.group(1), m.group(2).strip()
            if v.startswith('"') and v.endswith('"'):
                v = v[1:-1]
            data[k] = v
    return data, body, fm_block


def expansion_block(title: str, excerpt: str, slug: str) -> str:
    return rf"""

{PH1}

**{title}** — Ce chapitre prolonge l’angle « {excerpt} » pour le sujet réel derrière `{slug}`. L’objectif n’est pas d’empiler des adjectifs, mais d’installer une **boucle QA** courte que tu peux réutiliser sur chaque livrable : capture, note, compare, tranche, archive. La plupart des créateurs perdent du temps parce qu’ils mélangent trois variables en une session, puis blâment le modèle. Quand tu sépares lumière, composition, texture, intention, tu retrouves un diagnostic honnête et une progression mesurable.
...
Pour `{slug}`, retiens trois lignes dans ton carnet : intention en une phrase, loi lumière en une phrase, preuve matérielle en une phrase. Si l’une manque, tu n’es pas prêt à regénérer massivement : tu es prêt à diagnostiquer. La qualité long terme vient de cette discipline, pas du dernier modèle sorti mardi.
""".strip()


def phase2_block(title: str, excerpt: str, slug: str) -> str:
    """Deuxième passe longue pour franchir 2000 mots quand la passe 1 ne suffit pas."""
    return rf"""

{PH2}

**{title}** — L’extrait « {excerpt} » pose souvent une attente implicite : un livrable stable, défendable, reproductible. Le slug `{slug}` sert de fil conducteur : chaque export doit pouvoir être relié à une intention, une preuve, une limite. Cette section ajoute une couche **gouvernance + risques + livrables** que tu peux recopier dans ton Notion interne ou ton drive projet.
...
Tu n’as pas besoin d’analytics complexes : compte le **temps moyen par itération**, le **taux d’abandon** (images jetées), et le **taux de validation du premier essai**. Si le premier essai est toujours rejeté, ton brief est probablement flou. Si tu jettes tout, ton protocole mélange trop de variables. Pour **{title}**, ces métriques te disent si tu progresses ou si tu te déplaces latéralement.

### Escalade qualité : quand arrêter de regénérer

Arrête quand tu corriges un détail qui n’apparaît qu’à 400 % de zoom, sauf usage print géant. Arrête quand la géométrie est bonne mais que seule une micro-texture gêne : passe en post ciblée. Arrête quand tu changes de modèle pour fuir un problème de lumière : tu réinitialises tout le reste. Le slug `{slug}` doit rester un **projet maîtrisé**, pas une spirale.

### Archivage : ce qu’un futur toi remerciera

Archive : prompts principaux (même partiels), **deux captures** A/B annotées, la **liste des outils** et versions, et une phrase « pourquoi on a tranché ainsi ». Si tu livres à un client, un zip propre avec README court vaut mieux que dix fichiers mal nommés. Pour l’angle « {excerpt} », l’archive prouve que tu as suivi un processus, pas seulement une intuition du moment.
...
Pour **{title}** et le périmètre `{slug}`, retiens : livrable = paquet, risque = trace écrite, gouvernance = rôles et décisions datées. L’extrait « {excerpt} » devient actionnable quand tu relies chaque phrase du brief à une preuve visuelle ou à une limite assumée. Ce n’est pas du pessimisme : c’est ce qui permet de livrer vite **sans** regret.
""".strip()
...
def apply_enrichment(
    ...
    *,
    ...
    """Retourne (nouveau corps, modifié)."""
    n0 = word_count_body(body)
    if n0 >= MIN_WORDS:
        return body, False
    new_body = body
    changed = False
    if MARKER in new_body:
        if PH1 not in new_body:
            new_body = new_body.replace(
                MARKER,
                expansion_block(title, excerpt, slug) + "\n\n" + MARKER,
                1,
            )
            changed = True
        if word_count_body(new_body) < MIN_WORDS and PH2 not in new_body:
            new_body = new_body.replace(
                MARKER,
                phase2_block(title, excerpt, slug) + "\n\n" + MARKER,
                1,
            )
            changed = True
    else:
        # Article sans section « Vidéo YouTube à intégrer » : enrichir en fin de texte.
        if PH1 not in new_body:
...
def main() -> None:
    ...
    for path in sorted(BLOG.glob("*.md")):
        ...
        if not parsed:
            continue
        fm, body, fm_block = parsed
        title = fm.get("title", path.stem.replace("-", " "))
        excerpt = fm.get("excerpt", "")
        slug = path.stem
        n0 = word_count_body(body)
        new_body, changed = apply_enrichment(
            body,
            title=title,
            excerpt=excerpt,
            slug=slug,
        ...
        new_n = word_count_body(new_body)
        print(f"{path.name}: {n0} -> {new_n}")
        updated += 1
