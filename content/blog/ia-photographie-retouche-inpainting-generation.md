...
## Retouche : ce qui reste photographiquement honnête

Balance exposition locale, poussière capteur, petites distractions, harmonisation colorimétrique globale. L’objectif : rapprocher la scène de ce que l’œil humain aurait perçu sur place, sans ajouter une action qui n’a pas eu lieu.

## Inpainting : le masque est ton contrat

Définis la zone **au pixel près**. Documente avant / après. Si tu modifies une zone avec du texte ou des visages reconnaissables, augmente le niveau de validation.
...
Dès que tu inventes un sujet absent du shoot, tu es en **image de synthèse**. Ce n’est pas « mieux » ou « pire », mais l’étiquette change. Presse, pub comparative, preuve judiciaire : les exigences diffèrent.

## Chaîne de fichiers

Garde le **RAW** intact, une version TIFF master retouchée, une version web. Note l’outil IA et la version dans un fichier texte `readme` dossier.
...
## Tableau : action, transparence minimale, risque

| Action | Transparence | Risque |
| --- | --- | --- |
| balance globale | faible | faible |
| retirer fil électrique | moyenne | moyenne |
| reconstruire arrière-plan | élevée | confusion documentaire |
| visage « amélioré » | très élevée | éthique / droit image |

## Approfondissement terrain : L’IA au service de la photographie : retouche, inpainting et génération

Ce chapitre prolonge l’angle « Quand l’IA aide sans mentir : nettoyage local, extension de cadrage, et génération contrôlée avec garde-fous éthiques. » pour le sujet réel derrière `ia-photographie-retouche-inpainting-generation`. L’objectif n’est pas d’empiler des adjectifs, mais d’installer une **boucle QA** courte que tu peux réutiliser sur chaque livrable : capture, note, compare, tranche, archive. La plupart des créateurs perdent du temps parce qu’ils mélangent trois variables en une session, puis blâment le modèle. Quand tu sépares lumière, composition, texture, intention, tu retrouves un diagnostic honnête et une progression mesurable.
...
Tu n’as pas besoin d’analytics complexes : compte le **temps moyen par itération**, le **taux d’abandon** (images jetées), et le **taux de validation du premier essai**. Si le premier essai est toujours rejeté, ton brief est probablement flou. Si tu jettes tout, ton protocole mélange trop de variables. Pour **L’IA au service de la photographie : retouche, inpainting et génération**, ces métriques te disent si tu progresses ou si tu te déplaces latéralement.

### Escalade qualité : quand arrêter de regénérer

Arrête quand tu corriges un détail qui n’apparaît qu’à 400 % de zoom, sauf usage print géant. Arrête quand la géométrie est bonne mais que seule une micro-texture gêne : passe en post ciblée. Arrête quand tu changes de modèle pour fuir un problème de lumière : tu réinitialises tout le reste. Le slug `ia-photographie-retouche-inpainting-generation` doit rester un **projet maîtrisé**, pas une spirale.

### Archivage : ce qu’un futur toi remerciera

Archive : prompts principaux (même partiels), **deux captures** A/B annotées, la **liste des outils** et versions, et une phrase « pourquoi on a tranché ainsi ». Si tu livres à un client, un zip propre avec README court vaut mieux que dix fichiers mal nommés. Pour l’angle « Quand l’IA aide sans mentir : nettoyage local, extension de cadrage, et génération contrôlée avec garde-fous éthiques. », l’archive prouve que tu as suivi un processus, pas seulement une intuition du moment.
...
Pour **L’IA au service de la photographie : retouche, inpainting et génération** et le périmètre `ia-photographie-retouche-inpainting-generation`, retiens : livrable = paquet, risque = trace écrite, gouvernance = rôles et décisions datées. L’extrait « Quand l’IA aide sans mentir : nettoyage local, extension de cadrage, et génération contrôlée avec garde-fous éthiques. » devient actionnable quand tu relies chaque phrase du brief à une preuve visuelle ou à une limite assumée. Ce n’est pas du pessimisme : c’est ce qui permet de livrer vite **sans** regret.
...
### L’IA remplace le photographe ?
Non. Elle remplace certaines tâches longues si bien pilotée.

### Lightroom suffit-il ?
Souvent pour retouche légère. Inpainting poussé passe par ailleurs.

### Puis-je étendre le cadrage pour Instagram ?
Oui, avec mention client si contractuellement requis.

### Le génératif casse-t-il la perspective ?
Parfois. Vérifie lignes de fuite et répétitions d’objets impossibles.

### Stock photo et IA ?
Lis les règles de chaque agence sur disclosure et preuve.

### Peau lissée IA ?
Zone sensible : garde texture crédible.

### Où apprendre SD inpainting ?
