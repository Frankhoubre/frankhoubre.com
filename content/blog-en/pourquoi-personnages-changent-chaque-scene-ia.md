---
title: "Why Do My Characters Change in Every Scene"
date: "2026-04-29"
dateModified: "2026-06-10"
category: "tutoriels"
excerpt: "Unstable prompt, uncontrolled seed, synonym costume, and absence of a sheet: continuity is a protocol, not a model option."
thumbnail: "/images/blog/pourquoi-personnages-changent-chaque-scene-ia/hero.webp"
---

If the face, the jacket, or the morphology slips as soon as you change shot, it is not "because the AI is bad". It is almost always because **the brief is not locked**: you rewrite the character on each prompt, you swap synonyms, or you change model with no archiving the winning line.

The complete method is here: [tutorial: consistent characters across several images](/en/blog/personnages-coherents-plusieurs-images-ia). This page summarizes the most frequent causes in shooting.

## Mistake 1: the identity block is not copy-pasted identically

You "reformulate" between two scenes. For a human, it is the same intention. For the model, these are two close characters.

**Fix:** a frozen **IDENTITY** paragraph, a variable **SCENE** paragraph. Never the opposite.

## Mistake 2: the seed is treated as magic

With no a stable prompt, the seed does not save. With a stable prompt, it helps to vary slightly, not to invent the continuity.

**Fix:** text log: prompt, seed, model, date.

![Workflow marker: sheet + frozen identity block.](/images/blog/pourquoi-personnages-changent-chaque-scene-ia/workflow-1.webp)

## Mistake 3: the costume has three different names

"Trench", "overcoat", "long coat": three token distributions.

**Fix:** a single material word + a color + a signature detail (button, zipper).

## Mistake 4: you change tool between shot A and shot B

Same brief, different engine: different distribution.

**Fix:** lock the chain for a sequence, document the conversion if you have to change.

## Table: signal, cause, fix

| Signal | Cause | Fix |
| --- | --- | --- |
| different nose | unstable prompt | identity block |
| shorter hair | synonyms | single term |
| age floats | vague adjectives | fixed apparent age |
| costume changes | non-repeated detail | signature object |

## Field deep dive: why do my characters change in every scene

This chapter extends the angle "Unstable prompt, uncontrolled seed, synonym costume, and absence of a sheet: continuity is a protocol, not a model option." for the real subject behind `pourquoi-personnages-changent-chaque-scene-ia`. The goal is not to pile up adjectives, but to install a short **QA loop** you can reuse on every deliverable: capture, note, compare, decide, archive. Most creators lose time because they mix three variables in one session, then blame the model. When you separate light, composition, texture, intention, you get back an honest diagnosis and a measurable progression.
...
For `pourquoi-personnages-changent-chaque-scene-ia`, keep three lines in your notebook: intention in one sentence, light law in one sentence, material proof in one sentence. If one is missing, you are not ready to regenerate massively: you are ready to diagnose. Long-term quality comes from this discipline, not from the latest model released on Tuesday.

## Series B extension: deliverables, risks and governance

**Why do my characters change in every scene**: The excerpt "Unstable prompt, uncontrolled seed, synonym costume, and absence of a sheet: continuity is a protocol, not a model option." often sets an implicit expectation: a stable, defensible, reproducible deliverable. The slug `pourquoi-personnages-changent-chaque-scene-ia` serves as a guiding thread: each export must be traceable to an intention, a proof, a limit. This section adds a **governance + risks + deliverables** layer you can copy into your internal Notion or your project drive.
...
For **Why do my characters change in every scene** and the scope `pourquoi-personnages-changent-chaque-scene-ia`, keep: deliverable = package, risk = written trace, governance = roles and dated decisions. The excerpt "Unstable prompt, uncontrolled seed, synonym costume, and absence of a sheet: continuity is a protocol, not a model option." becomes actionable when you link each sentence of the brief to a visual proof or to an owned limit. This is not pessimism: it is what lets you deliver fast **without** regret.

![Second marker: seeds and versions log.](/images/blog/pourquoi-personnages-changent-chaque-scene-ia/workflow-2.webp)

## FAQ

### Do the hands break too?
Often: [how to avoid distorted faces in AI generation](/en/blog/comment-eviter-visages-deformes-generation-ia) and hand framing.

### Is LoRA mandatory?
No but useful when the face refuses to stabilize.

### Video and character?
Same sheet, fewer changes between keyframes.

### Dzine or dedicated tools?
[Dzine: consistent characters](/en/blog/dzine-ia) can help according to your pipeline.

### Several characters?
One sheet per head, contrasted clothing colors.

### Style and identity?
[how to control the visual style in an AI generation](/en/blog/comment-controler-style-visuel-generation-ia).
